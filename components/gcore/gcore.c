...
bool gcore_get_reg8(uint8_t offset, uint8_t* dat)
{
	...
	if (i2c_master_write_slave(GCORE_I2C_ADDR, buf, 2) != ESP_OK) {
		i2c_unlock();
		ESP_LOGE(TAG, "failed to write byte register address %02x", offset);
		return false;
	}
	...
	if (i2c_master_read_slave(GCORE_I2C_ADDR, buf, 1) != ESP_OK) {
		i2c_unlock();
		ESP_LOGE(TAG, "failed to read from byte register %02x", offset);
		return false;
	}
...
bool gcore_set_reg8(uint8_t offset, uint8_t dat)
{
	...
	if (i2c_master_write_slave(GCORE_I2C_ADDR, buf, 3) != ESP_OK) {
		i2c_unlock();
		ESP_LOGE(TAG, "failed to write byte register %02x = %2x", offset, dat);
		return false;
	}
...
bool gcore_get_reg16(uint8_t offset, uint16_t* dat)
{
	...
	if (i2c_master_write_slave(GCORE_I2C_ADDR, buf, 2) != ESP_OK) {
		i2c_unlock();
		ESP_LOGE(TAG, "failed to write word register address %02x", offset);
		return false;
	}
	...
	if (i2c_master_read_slave(GCORE_I2C_ADDR, buf, 2) != ESP_OK) {
		i2c_unlock();
		ESP_LOGE(TAG, "failed to read from word register %02x", offset);
		return false;
	}
...
bool gcore_set_reg16(uint8_t offset, uint16_t dat)
{
	...
	if (i2c_master_write_slave(GCORE_I2C_ADDR, buf, 4) != ESP_OK) {
		i2c_unlock();
		ESP_LOGE(TAG, "failed to write word register %02x = %04x", offset, dat);
		return false;
	}
...
bool gcore_set_nvram_byte(uint16_t offset, uint8_t dat)
{
	...
	if (i2c_master_write_slave(GCORE_I2C_ADDR, buf, 3) != ESP_OK) {
		i2c_unlock();
		ESP_LOGE(TAG, "failed to write NVRAM %02x = %2x", offset, dat);
		return false;
	}
...
bool gcore_get_nvram_bytes(uint16_t offset, uint8_t* dat, uint16_t len)
{
	...
	if (i2c_master_write_slave(GCORE_I2C_ADDR, buf, 2) != ESP_OK) {
		i2c_unlock();
		ESP_LOGE(TAG, "failed to write NVRAM register address %04x", offset);
		return false;
	}
	...
	if (i2c_master_read_slave(GCORE_I2C_ADDR, dat, len) != ESP_OK) {
		i2c_unlock();
		ESP_LOGE(TAG, "failed to read %d bytes from NVRAM %04x", len, offset);
		return false;
	}
...
bool gcore_set_nvram_bytes(uint16_t offset, uint8_t* dat, uint16_t len)
{
	...
	if (i2c_master_write_slave(GCORE_I2C_ADDR, buf, len+2) != ESP_OK) {
		i2c_unlock();
		free(buf);
		ESP_LOGE(TAG, "failed to write %d bytes to NVRAM %04x", len, offset);
		return false;
	}
